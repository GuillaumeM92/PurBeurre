...
class App:
    ...
    def get_product_category(self, cat_choice):
        self.cursor.execute(
            'SELECT nom_catégorie FROM pur_beurre_catégories WHERE numéro_catégorie = {0}'.format(cat_choice))
    ...
    def return_result(product_data, cat_choice):
        ...
        print('Note nutritionnelle (A = meilleure, E = pire) : {0}'.format(
            product_data[2]))
                ...
                if choice is 1:
                    ...
                    if cat_choice > 0:
                        if cat_choice <= self.db.categories_limit:
                            ...
                            self.return_result(self.browse_db(
                                food_choice), self.get_product_category(cat_choice))

                            self.get_suggestion()
                            self.return_result(self.browse_db_suggestion(
                                cat_choice), self.get_product_category(cat_choice))
                            self.store_suggestion(
                                self.browse_db_suggestion(cat_choice))
