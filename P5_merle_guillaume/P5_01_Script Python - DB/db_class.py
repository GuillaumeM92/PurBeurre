...
class DataBase:
    ...
    def __init__(self, my_host, my_user, my_password, db_name):
        self.db = mysql.connector.connect(host = my_host, user = my_user, password = my_password)
        self.name = db_name
        self.cursor = self.db.cursor(buffered = True)   # creating an instance of the 'cursor' class to execute SQL statements from Python
        self.json_db = requests.get('https://fr.openfoodfacts.org/cgi/search.pl?action=process&page_size=1000&json=True').json()
        self.json_categories = requests.get('https://fr.openfoodfacts.org/categories&json=True').json()
        self.json_categories_list = []
        self.categories_limit = 28

    def drop(self):
        self.cursor.execute('DROP DATABASE IF EXISTS {0}'.format(self.name))   # drops the previous db with same name if exists
    ...
    def inject_products(self):  # reads the json data and inserts the desired values in the DB
    ...
    def inject_link_table(self):
        for product in self.json_db['products']:
            try:
                product_categories_list = product['categories'].replace("' ", "'").replace(", ", ",").split(',')
                ...
                for category in product_categories_list:
                    if category in self.json_categories_list:
                        try:
                            ...
                            self.cursor.execute('SELECT DISTINCT numéro_produit FROM pur_beurre_produits WHERE nom_aliment = "{0}" LIMIT 1'.format(product_name))
                            resp = self.cursor.fetchone()
                            insert = 'INSERT INTO pur_beurre_catégories_produits (numéro_produit, numéro_catégorie) VALUES (%s, %s)'
                            values = (resp[0], (self.json_categories_list.index(category)) + 1)
                            self.cursor.execute(insert, values)
                            continue
...
db = DataBase('localhost', 'root', 'Veanip206', 'pur_beurre_db')
